...
public class LoginSystem : MonoBehaviour {
		...
		ParseUser.LogInAsync(nickInputLogin.value, nickInputLogin.value).ContinueWith(t => 
		{
			if(t.IsCanceled || t.IsFaulted)
			{
				...
				if(enumerator.MoveNext())
				{
					...
					{
						warningText = "Usuario nao encontrado. Solicite primeiro acesso";
					}
					else
	...
	IEnumerator CadastrarUsuario()
	{
		...
		Task task = newUser.SignUpAsync ();
		while(!task.IsCompleted)
			yield return null;
		if(task.IsFaulted || task.IsCanceled)
		{
			_4Label.text = "Falha ao Cadastrar.";
		}
		else
		{
			_4Label.text = "Usuario cadastrado\ncom sucesso!";
		}
